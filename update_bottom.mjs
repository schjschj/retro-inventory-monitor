        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-cyan-300 uppercase tracking-wider">
              실시간 운송 차수 전술 모니터 (총 {shipments.length} 개 차수)
            </span>
          </div>
          <button
            onClick={() => {
          ...
          <table className="w-full text-left text-[11px] border-collapse">
            <thead>
              <tr className="bg-[#101b2d] text-slate-300 border-b border-slate-700">
                <th className="p-2 font-bold">운송 구분</th>
                <th className="p-2 font-bold">차수명</th>
                <th className="p-2 font-bold">선박 / 편명</th>
                <th className="p-2 font-bold">컨테이너 / 식별</th>
                <th className="p-2 font-bold">출항일 ➔ ETA</th>
                <th className="p-2 text-right font-bold">적재 수량</th>
                <th className="p-2 font-bold">현재 진행 단계</th>
                <th className="p-2 w-32 font-bold">진행률</th>
              ...
              {filteredShipments.map((s) => (
                ...
                >
                  ...
                  <td className="p-2 text-slate-300 text-[10px]">
                    {s.progress <= 75 ? '태평양 해상 항해' : s.progress <= 96 ? '미 내륙 철도 환적' : '코코모 인근 도착'}
                  </td>
                  <td className="p-2">
                    <div className="flex items-center gap-2">
...
fs.writeFileSync(path.resolve('./src/components/BottomTransitTable.jsx'), tableContent, 'utf8');
console.log('BottomTransitTable.jsx updated safely.');
